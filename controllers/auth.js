...
const signup = async (req, res, next) => {
  try {
    ...
    if (isEmailExist) {
      res.code = 400;
      throw new Error("This is email is existed already");
    }
    ...
    res
      .status(201)
      .json({ code: 200, status: true, message: "User Created Succesfully" });
  } catch (error) {
    next(error);
  }
};
...
const verifyCode = async (req, res, next) => {
  try {
    ...
    //user.isVerified === true equal to user.isVerified
    if (user.isVerified) {
      res.code = 400;
    ...
    await sendEmail({
      emailTo: user.email,
      subject: "Email Verificaton Code",
      code,
...
const verifyUser = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      code: 200,
      status: true,
      message: "User Verification Successed ",
    });
  } catch (error) {
    next(error);
  }
...
const changePassword = async (req, res, next) => {
  try {
    ...
    if (oldPassword === newPassword) {
      res.code = 400;
      throw new Error("Old password must not equal to new password");
    }
